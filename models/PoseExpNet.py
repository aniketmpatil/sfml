...
def conv(in_planes, out_planes, kernel_size=3):
    '''
        Convolution common for Pred and Explanability Mask
    '''
...
def upconv(in_planes, out_planes):
    '''
        Upconv for Explainability Mask
    '''
    return nn.Sequential(
...
class PoseExpNet(nn.Module):
    def __init__(self, nb_ref_imgs=2, output_exp=False):
        ...
        self.nb_ref_imgs = nb_ref_imgs                  # Default = 3, in train.py args.parser

        conv_planes = [16, 32, 64, 128, 256, 256, 256]
        self.conv1 = conv(3*(1+self.nb_ref_imgs), conv_planes[0], kernel_size=7)     ## Input channels??
    ...
    def forward(self, target_image, ref_imgs):
        ...
        pose = pose.mean(3).mean(2)     ## ?? tensor.mean()
        pose = 0.01 * pose.view(pose.size(0), self.nb_ref_imgs, 6)      ## reshape using tensor.view
    
        if self.output_exp:
            out_upconv5 = self.upconv5(out_conv5  )[:, :, 0:out_conv4.size(2), 0:out_conv4.size(3)]        ## Why size needs to be matched
