...
class GeoNetModel(object):
    def __init__(self, config, train_flow):
        ...
        self.disp_net = models.DispNetS()
        self.pose_net = models.PoseExpNet(self.num_source)
        '''input channels:
            #src_views * (3 tgt_rgb + 3 src_rgb + 3 warp_rgb + 2 flow_xy +1 error )
        '''
        self.flow_net = models.FlowNet(12, self.config['flow_scale_factor'])
        if device.type == 'cuda':
            self.disp_net.cuda()
            self.pose_net.cuda()
            self.flow_net.cuda()
        ...
        if not self.train_flow and (not os.path.isfile(
            ...
            weights = torch.load('saved_models/dispnet_model_best.pth.tar')
            self.disp_net.load_state_dict(weights['state_dict'])
            weights = torch.load('saved_models/exp_pose_model_best.pth.tar')
            self.pose_net.load_state_dict(weights['state_dict'])
    ...
    def iter_data_preparation(self, sampled_batch):
        ...
        self.src_views = [src.to(device) for src in src_views]
